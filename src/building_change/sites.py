...
def _records(collection: dict[str, Any], *, role: str) -> list[dict[str, Any]]:
...
def build_sites(
    ...
    *,
    ...
    changes = _records(change_collection, role="change")
    footprints = _records(footprint_collection, role="footprint")
    ...
    features = [_site_feature(index, cluster, attached[index - 1]) for index, cluster in enumerate(clusters, start=1)]
    features.extend(_site_feature(len(features) + index, [], [footprint]) for index, footprint in enumerate(unassigned, start=1))
