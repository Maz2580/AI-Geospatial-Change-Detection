...
def main():
    ...
    out_path = "../../data/output/dinov2_change_map.tif"
    ...
    with rasterio.open(t1_path) as src1, rasterio.open(t2_path) as src2:
        ...
        for row in tqdm(range(0, height, patch_size), desc="Rows"):
            for col in range(0, width, patch_size):
                ...
                change_map[window.row_off:window.row_off+window.height, window.col_off:window.col_off+window.width] = (change_mask * 255).astype(np.uint8)
